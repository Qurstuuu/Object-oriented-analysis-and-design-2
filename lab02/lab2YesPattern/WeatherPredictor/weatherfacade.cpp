...
WeatherFacade::WeatherFacade(QObject *parent)
    : QObject(parent)
    ...
    , m_timeoutTimer(new QTimer(this))
{
    ...
    connect(m_timeoutTimer, &QTimer::timeout, this, &WeatherFacade::onTimeout);

    m_memoryCache.setMaxCost(10);
}

void WeatherFacade::requestWeather(const QString &cityName)
{
    if (cityName.trimmed().isEmpty()) {
        emit errorOccurred("Введите название города");
        return;
    }
    emit loadingStarted();

    WeatherData cachedData;
    if (loadFromCache(cityName, cachedData)) {
        emit weatherReceived(cachedData);
        emit loadingFinished();
        return; // нашли в кэше
    }

    m_timeoutTimer->start();
    QString geocodeUrl = buildYandexGeocodeUrl(cityName);
    QUrl url(geocodeUrl);
...
QString WeatherFacade::buildYandexGeocodeUrl(const QString &cityName)
{
...
bool WeatherFacade::parseYandexGeocode(const QByteArray &jsonData,
...
void WeatherFacade::onGeocodeReply(QNetworkReply *reply)
{
    m_timeoutTimer->stop();
    ...
    if (reply->error() != QNetworkReply::NoError) {
        emit errorOccurred("Город \"" + cityName + "\": " + reply->errorString());
        reply->deleteLater();
...
QString WeatherFacade::buildWeatherUrl(double lat, double lon)
{
...
bool WeatherFacade::parseWeather(const QByteArray &jsonData, WeatherData &data)
{
    QJsonDocument doc = QJsonDocument::fromJson(jsonData);
    if (doc.isNull() || !doc.isObject()) {
...
void WeatherFacade::onWeatherReply(QNetworkReply *reply)
{
    ...
    if (parseWeather(data, weather)) {
        ...
        emit errorOccurred("Не удалось получить данные о погоде для \"" + cityName + "\"");
    }
...
bool WeatherFacade::loadFromCache(const QString &city, WeatherData &data)
{
    ...
    QString key = "weather_" + city.toLower();
    if (m_diskCache->contains(key)) {
        ...
        if (!doc.isNull() && doc.isObject()) {
            ...
            if (secondsPassed < CACHE_TIMEOUT_MINUTES * 60) {
                ...
                qDebug() << "Данные для" << city << "устарели";
...
void WeatherFacade::saveToCache(const WeatherData &data)
{
    m_memoryCache.insert(data.cityName, new WeatherData(data));
    ...
    QString key = getCacheKey(data.cityName);
    m_diskCache->setValue(key, doc.toJson());

    qDebug() << "Сохранено в кэш:" << data.cityName;
}

QString WeatherFacade::getCacheKey(const QString &city)
{
    return "weather_" + city.toLower();
}

QString WeatherFacade::getWindDirection(double degrees)
{
...
void WeatherFacade::onTimeout()
{
    emit errorOccurred("Превышено время ожидания ответа от сервера");
    emit loadingFinished();
}
